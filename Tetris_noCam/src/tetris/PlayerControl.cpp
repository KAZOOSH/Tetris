...
void PlayerControl::initSerial()
{
	...
	if (!deviceDescriptors.empty())
	{
		...
		for (auto deviceDescriptor : deviceDescriptors)
		{
			...
			if (deviceDescriptor.getHardwareId() == components->params()->settings["serial"]["buzzerId"].get<string>()) {
				connectDevice(serialBuzzer, deviceDescriptor,"buzzer");
			}else if (deviceDescriptor.getHardwareId() == components->params()->settings["serial"]["pedalId"].get<string>()) {
