...
CamInput::CamInput(string moduleName):ModuleDrawable("CamInput",moduleName){
    ...
    gui.add(contourMinAreaRadius.set("contourMinAreaRadius", 6, 1, 150));
...
void CamInput::sendPositions()
{
    ...
    ofJson send;
    send["function"] = "paddle" + nPaddle.get() + "Position";
    send["paddle"] =
    { {
        { "x", ofMap(foundPoints[2].x,0,width,1.0,0.0) },
        { "y", ofMap(foundPoints[2].y,0,height,0.0,1.0) }
    },{
        { "x", ofMap(foundPoints[0].x,0,width,1.0,0.0) },
